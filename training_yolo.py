...
def train_folds(base_path): #code qui fait le training sur les images 
    print(base_path)
    
    #models = ["yolov8s-seg"]
    models = ["yolo11x-seg"]
    # models = ["yolov8n-seg", "yolov8s-seg", "yolov8m-seg", "yolov8l-seg"] #,"yolov8x-seg" #selectionner les modèles à entrainer 
    # models = ["yolov8n-seg", "yolov8m-seg"]
    # models = ["yolov8l-seg","yolov8x-seg"]
    base_path = Path(base_path) #path vers le dossier d'image 
    ...
    if subfolder.is_dir():
        for m in models:
            for i in range(1, 6):  # Itérer de fold1 à fold5
                ...
                if fold_path.exists():
                    model = YOLO(f'{m}.pt')      
                    model.train(  #paramètre de training à choisir: 
                        ...
                        pretrained=False,
                        cache=False,
                        project=f"{m}/{subfolder.name}/fold_{i}"  #nom du dossier où les résultats sont sauvés 
...
if __name__ == "__main__":  #indiquer le chemin vers le dataset sur lequel on entraine notre modele !!fichier yamls!! 
    base_path = Path("C:\\Users\\Haroun\\Desktop\\YOLO_CROSS_VALIDATION\\AZHWoundsDB_yamls")  #Path(".\\yamls\\")
